...
with tab🌬 MAIN TANK VENT1:
    ...
    for config, col in zip(id_configs, col_sets):
        with col:
            ...
            developed_length = st.number_input(f"{label} Developed Length (ft)", min_value=0.0, value=0.0, step=1.0, key=f"dev_{label}_{TAB_ID}")

            st.markdown("---")
            def fitting_input(tag, multiplier):
                qty = st.number_input(f"{label} {tag} (qty)", min_value=0, value=0, step=1, key=f"{tag}_{label}_{TAB_ID}")
            ...
            knockout_le_total = 0
            for i in range(3):
                d = st.number_input(f"{label} Knockout {i+1} Diameter (in)", min_value=0.0, value=0.0, key=f"kdiam{i}_{label}_{TAB_ID}")
            ...
            for i in range(3):
                cv = st.number_input(f"{label} Specialty Valve {i+1} Cv", min_value=0.0, value=0.0, key=f"cv{i}_{label}_{TAB_ID}")
