...
# -----------------------------
# Shared Vent Tab Logic Below - Updated version with summary moved
# -----------------------------
def render_vent_tab(label_prefix):
    ...
    col_summary = st.columns(len(id_configs))
    for config, col in zip(id_configs, col_summary):
        with col:
            ID_in = config["id_in"]
            label = config["label"]
            st.metric(f"{label_prefix} {label} Total Length (ft) of 3\" NPS", f"0.00")  # Placeholder
            summary_lengths.append(0.0)  # Placeholder
